  ...
  return async (req, res) => {
    ...
    var boundHandler = local.bind(async () => {
        ...
        if (tracing.hasTracingEnabled()) {
          ...
          if (req.headers && utils.isString(req.headers['sentry-trace'])) {
            ...
            flags.IS_DEBUG_BUILD &&
              utils.logger.log(
                `[Tracing] Continuing trace ${_optionalChain([traceparentData, 'optionalAccess', _ => _.traceId])}.`,
              );
          }

          var url = `${req.url}`;
          // pull off query string, if any
          let reqPath = utils.stripUrlQueryAndFragment(url);
      ...
      try {
        ...
        var objectifiedErr = utils.objectify(e);
...
async function finishSentryProcessing(res) {
  ...
  try {
    flags.IS_DEBUG_BUILD && utils.logger.log('Flushing events...');
    await node.flush(2000);
    flags.IS_DEBUG_BUILD && utils.logger.log('Done flushing events');
  } catch (e) {
    flags.IS_DEBUG_BUILD && utils.logger.log('Error while flushing events:\n', e);
  }
}
